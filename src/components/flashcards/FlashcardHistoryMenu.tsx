import { BookMarked, Layers, Trash2 } from "lucide-react";
import clsx from "clsx";
import type { SavedDeck } from "../../store/flashcardStore";
...
export function FlashcardHistoryMenu({ items, activeId, onOpen, onDelete, onClearAll }: Props) {
  return (
    <div className="w-full rounded-2xl border border-ink-3 bg-ink-2 shadow-2xl overflow-hidden flex flex-col">
      <div className="px-4 pt-3 pb-2">
        <p className="text-sm font-display font-semibold">Saved flashcards</p>
      </div>

      {items.length === 0 ? (
        <p className="px-4 pb-5 pt-2 text-sm text-paper/50">
          Nothing saved yet. Generate flashcards and they'll be saved here automatically.
        </p>
      ) : (
          ...
          {items.map((item) => {
            ...
            return (
              <li key={item.id} className="group relative">
                ...
                >
                  <Layers className={clsx("w-4 h-4 mt-0.5 flex-shrink-0", isActive ? "text-signal" : "text-paper/40")} />
                  <span className="min-w-0">
                    ...
                    <span className="block text-[11px] text-paper/40">
                      {total} card{total === 1 ? "" : "s"} · at card {Math.min(item.position + 1, total)}
                    </span>
                  </span>
                </button>
                <button
                  ...
                  aria-label={`Delete flashcards for ${item.lessonTitle}`}
